...
class TaskScheduler:
    ...
    async def __new_context(self, task: Union[ScheduledTask, AdHocTask, PlannedTask, "OneShotTask"]) -> AgentContext:
        ...
        if task.agent_profile:
            ...
            config.profile = settings.get_settings()["agent_profile"]
    ...
    async def _get_chat_context(self, task: Union[ScheduledTask, AdHocTask, PlannedTask, "OneShotTask"]) -> AgentContext:
        ...
        if context:
            ...
            # Reconfigure context and agent chain to match task's agent_profile if provided
            try:
                desired_profile = task.agent_profile or settings.get_settings()["agent_profile"]
                if desired_profile is not None:
                    # Rebuild config and propagate profile to agents
                    new_config = initialize_agent()
                    new_config.profile = desired_profile
                    context.config = new_config
                    agent = context.streaming_agent or context.agent0
                    while agent:
                        agent_profile_name = getattr(agent.config, "profile", desired_profile)
                        new_agent_config = initialize_agent()
                        new_agent_config.profile = agent_profile_name
                        agent.config = new_agent_config
