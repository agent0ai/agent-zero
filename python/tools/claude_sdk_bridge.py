"""
Claude SDK Bridge Tool for Agent Zero
...
class ClaudeSDKBridge(Tool):
    """
    Agent Zero tool for Claude Agent SDK integration.
    ...
    async def _init_sdk(self):
        ...
        if "error" in result:
            return Response(
                message=f"Session initialization failed: {result['error']}",
                break_loop=False
            )
    ...
    async def _close_sdk(self):
        ...
        return Response(
            message=f"SDK session {result['status']}.",
            break_loop=False
        )
    ...
    async def _cli_query(self):
        ...
        if "error" in result:
            return Response(
                message=f"CLI query failed: {result['error']}",
                break_loop=False
            )
    ...
    async def _export_tool(self):
        """Export an Agent Zero tool for SDK use"""
        tool_name = self.args.get("tool_name")

        if not tool_name:
            return Response(
                message="Error: tool_name is required",
                break_loop=False
            )
        ...
        try:
            ...
            tool_classes = load_classes_from_folder(
                files.get_abs_path("python/tools"),
                name_pattern=f"*{tool_name}*"
            )
    ...
    async def _get_mcp_config(self):
        ...
        if not server_name or not command:
            return Response(
                message="Error: server_name and command are required",
                break_loop=False
            )
    ...
    async def _bridge_mcp(self):
        ...
        if not server_name:
            return Response(
                message="Error: server_name is required",
                break_loop=False
            )
        ...
        if "error" in result:
            return Response(
                message=f"Bridge failed: {result['error']}",
                break_loop=False
            )
