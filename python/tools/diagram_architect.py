"""
Diagram Architect Tool for Agent Zero
...
class DiagramArchitect(Tool):
    """
    Agent Zero tool for architecture analysis and diagram generation.
    ...
    async def _get_analysis(self):
        ...
        if not analysis_id:
            return Response(
                message="Error: analysis_id is required",
                break_loop=False
            )
        ...
        return Response(
            message=self._format_result(result, f"Analysis: {result['project_name']}"),
            break_loop=False
        )
    ...
    async def _generate_from_app_spec(self):
        ...
        if not app_spec_path:
            return Response(
                message="Error: app_spec_path is required",
                break_loop=False
            )
    ...
    async def _get_diagram(self):
        ...
        if not diagram_id:
            return Response(
                message="Error: diagram_id is required",
                break_loop=False
            )
    ...
    def _format_diagram_response(self, result: dict, diagram_name: str) -> Response:
        ...
        if "error" in result:
            return Response(
                message=f"Error generating {diagram_name}: {result['error']}",
                break_loop=False
            )
