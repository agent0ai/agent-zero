...
- Finished response logs that embed local `img://` image refs must snapshot those files onto unique chat artifacts before persistence so reused workdir filenames cannot overwrite earlier bubbles.
