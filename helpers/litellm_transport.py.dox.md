...
- Streaming Chat Completions requests include `stream_options.include_usage` so terminal usage events reach `LLMResult.usage` for OpenAI-compatible endpoints and Messages API providers alike. A caller-provided `stream_options` keeps its extra keys, and a pre-output rejection of the option retries once without `include_usage`, remembering the rejection per model configuration in a process-global cache that naturally clears when the framework process restarts; tests reset it through `clear_transport_capability_cache`.
