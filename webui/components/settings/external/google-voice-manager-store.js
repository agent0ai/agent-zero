...
const model = {
  ...
  loadAutoSend() {
    const field = this._getSettingsFields().find((entry) => entry.id === "google_voice_auto_send");
    if (field) {
  ...
  loadShortcuts() {
    const field = this._getSettingsFields().find((entry) => entry.id === "google_voice_shortcuts");
    if (field) {
  ...
  loadTagPresets() {
    const field = this._getSettingsFields().find((entry) => entry.id === "google_voice_tag_filters");
    if (field) {
      try {
  ...
  getContactRules() {
    const field = this._getSettingsFields().find((entry) => entry.id === "google_voice_contact_rules");
