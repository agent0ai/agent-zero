...
> [!TIP]
> For OpenAI-compatible providers (e.g., custom gateways or Z.AI/GLM), add the API key under **External Services -> Other OpenAI-compatible API keys**, then select **OpenAI Compatible** as the provider in model settings. One concrete multi-model gateway example is [DaoXE](https://daoxe.com/) at `https://api.daoxe.com/v1` (set as the API URL / base URL).
