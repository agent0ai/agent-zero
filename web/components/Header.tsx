    ...
    <header className="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <ul className="hidden md:flex gap-8">
          ...
          <li>
            <a href="https://github.com/agent-zero-deploy/agent-jumbo" className="text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white" onClick={() => trackExternalClick('GitHub')}>
              GitHub
            </a>
          </li>
        </ul>
      ...
      {isOpen && (
        <nav className="md:hidden border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 space-y-2">
            ...
            <a href="https://github.com/agent-zero-deploy/agent-jumbo" className="block px-3 py-2 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 rounded" onClick={() => { trackExternalClick('GitHub'); setIsOpen(false) }}>
              GitHub
            </a>
          </div>
        </nav>
