...
## The Problem
Agent Zero is trying to use OpenRouter (default) but you haven't configured an API key. 
You need to switch it to use your local Ollama/Qwen model.

## The Solution - Configure Qwen NOW

### Step 1: Open Settings
1. In the Agent Zero UI (http://localhost:8080)
...
```bash
docker exec agent-zero curl -s http://host.docker.internal:11434/api/tags | head -20
```
