...
class RunExperiment(Tool):
    ...
    async def execute(
        ...
        **kwargs,
        ...
        """
        # Load idea
        idea = self.agent.context.data.get("research_ideas", {}).get(idea_name)
        if not idea:
            return Response(
    ...
    def _init_experiment_state(
        ...
        if "experiments" not in self.agent.context.data:
            self.agent.context.data["experiments"] = {}

        if resume and idea_name in self.agent.context.data["experiments"]:
            return self.agent.context.data["experiments"][idea_name]
        ...
        self.agent.context.data["experiments"][idea_name] = state
        return state
