...
- `looks_like_tool_call` returns True only when the transformed output is a dict containing at least one A0 key (thoughts/headline/tool_name/tool_args) with a usable non-empty value: non-empty list of non-empty strings for `thoughts`, non-empty string for `headline`/`tool_name`, non-empty dict for `tool_args`. Empty dicts, missing keys, or keys with empty/blank values fall through to the raw-text fallback branch.
