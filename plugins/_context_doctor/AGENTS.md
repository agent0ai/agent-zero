...
- `looks_like_tool_call` returns True when the transformed output is a dict containing at least one A0 key (thoughts/headline/tool_name/tool_args). Empty dicts or dicts without any A0 key fall through to the raw-text fallback branch.
