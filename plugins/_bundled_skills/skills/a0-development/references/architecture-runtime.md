...
| `plugins/_bundled_skills/skills/` | Bundled skills. |
...
Skill discovery is handled by `helpers.skills.get_skill_roots(...)`. Skills may come from the bundled `plugins/_bundled_skills/skills/` collection, user `usr/skills/`, project metadata, agent profile folders, and other plugin roots. Loaded skills can expose additional files via `skills_tool action=read_file`.
