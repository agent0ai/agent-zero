...
Use the authorized session/CSRF client from `/a0/plugins/_bundled_skills/skills/a0-development/references/operate-agent-zero.md`. Discover the live origin; do not assume an internal or published port. The session cookie and `X-CSRF-Token` header are sufficient; do not invent an additional required CSRF cookie. Preserve login and CSRF protections.
